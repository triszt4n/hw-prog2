...
String addString(const String& s0, char ch) {
    String chStr;
    createString(chStr, ch);

    String res = addString(s0, chStr);
    disposeString(chStr);
...
/**
 * Ostreamhez fűzi a Stringet, felhasználva a << operátor eredeti
 * definícióját.
 * @param o - bal oldali objektum
 * @param s0 - jobb oldali String
 */
std::ostream& operator<<(std::ostream& o, const String& s0) {
    o << s0.pData;
    return o;
}
